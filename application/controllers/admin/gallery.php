...
class Gallery extends CI_Controller {
	...
	public function view() {	
		...
		$list = $this->Gallery_Model->get_all_items();			
		$data['list'] = $list;
		$this->template->write_view("content","admin/gallery/gallery_list",$data);
	...
	{		
		...
		else
		{
			...
			$data['info'] = $this->Gallery_Model->get_items_info($id);
		}		
		//truyền dữ liệu ra form
		$this->template->write_view("content","admin/gallery/gallery_edit",$data);
		$this->template->render();
	}
	...
	public function save_update()
	{
		...
		{
			if($this->Gallery_Model->insert($image))
			{
		...
		{
			if($this->Gallery_Model->update($image))
			{
				$this->session->set_flashdata('message', '<div role="alert" class="alert alert-success"><button data-dismiss="alert" class="close" type="button">×</button>Lưu dữ liệu thành công !</div>');
				redirect('admin/gallery/view/');
			}
