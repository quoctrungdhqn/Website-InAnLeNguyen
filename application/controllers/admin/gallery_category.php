...
class Gallery_Category extends CI_Controller {
	...
	public function view() {	
		...
		$list = $this->Gallery_Category_Model->get_all_items();			
		$data['list'] = $list;
		$this->template->write_view("content","admin/gallery/gallery_category_list",$data);
	...
	{		
		...
		else
		{
			...
			$data['info'] = $this->Gallery_Category_Model->get_items_info($id);
		}		
		//truyền dữ liệu ra form
		$this->template->write_view("content","admin/gallery/gallery_category_edit",$data);
		$this->template->render();
	}
	...
	public function save_update()
	{
		...
		$upload_conf = array(
            'upload_path'   => realpath('./uploads/country/'),
		...
		{
			if($this->Gallery_Category_Model->insert($image))
			{
				...
				redirect('admin/gallery_category/view/');
			}
			else
			{
		...
		{
			if($this->Gallery_Category_Model->update($image))
			{
				$this->session->set_flashdata('message', '<div role="alert" class="alert alert-success"><button data-dismiss="alert" class="close" type="button">×</button>Lưu dữ liệu thành công !</div>');
				redirect('admin/gallery_category/view/');
			}
